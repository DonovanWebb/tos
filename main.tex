...
\section{Carrier Nulling Results}
    ...
    5S1/2 ↔ 4D5/2, in 88Sr+. The beams propogate in free-space and are
    phase-stabilized to ensure the SW is stationary with respect to
    the ion. Figure X displays the path of the two counter-propagating
    ...
    Figure X shows the dependence of carrier coupling with phase of
    ...
    a π-pulse at maximum carrier coupling, which occurs at the nodes
    ...
    sideband coupling, or vice versa, by selecting ∆φ = π or ∆φ = 0
    [Fig. 2(b)]. The transfer probability shown in Fig. 2(a) has a
    quartic dependence on ∆φ near ∆φ = π and a quadratic dependence at
    ∆φ = 0 [26]. When probing the suppressed motional sideband
    [Fig. 2(b) left], we observe only features that are due to the
    off-resonant (by ≈ 1.2 MHz) carrier coupling. By changing ∆φ of
    ...
    Identity was performed, known as the sequence fidelity. These
    ...
    utilizing either standing or travelling waves. We obtained an error of
    $1.44(3) \mult 10^{−3}$ and $1.73(3) \mult 10^{−3}$ per Clifford
    gate for the SW and TW single qubit gates, respectively. Thus, use
    of the SW is not detrimental to single-qubit gate fidelity. Due to
    spin dephasing of our quadrupole qubit originating both from
    natural lifetime and external noise, we ensured that the duty
    cycle and Rabi frequency of both SW and TW RBM experiments were
    the same. \\
    ...
    To explore the carrierThe two-qubit entangling
    gate is implemented via an MS-type scheme where we use a
    bichromatic SW instead of the conventional bichromatic TW. To
    selectively couple to the sidebands, and hence suppress the carrier term, the antinodes
    of the SW are placed at the ions.\\
...
COMET surface trap: depth = 59 meV. Ions sit at a distance d = 75 μm.\\
